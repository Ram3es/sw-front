...
export default function Inventory() {
  ...
  const [ cartObject, setCartObject ] = useState<Record<string, IOfferInventory[]> | undefined>()
  ...
  useEffect(() => {
    setCartObject(() => cartItems.items.reduce((prev: any, cur: IOfferInventory) => {
      if (prev[cur.appid]) {
                  ...
                  {cartObject[k].map(card => (
                    ...
                    >
                      <ItemSelectedCard
                        id={card.assetid}
                        variant='offer'
                        image={getImageURL(card.icon_url)}
                        price={0}
                        condition={0.03721566125750542}
                        name={card.name}
                        onClick={() => {}}
                      ...
                      <div className="flex flex-col justify-between pb-6">
                        <div className="flex flex-col items-center">
                          <div className="flex items-center gap-2">
                            <span className="text-white font-Barlow text-[21px] font-medium uppercase">
                              ${format(card.price.buy)}
                            </span>
                            <span className="px-0.5 bg-swLime text-xs text-darkSecondary">
                              -{((card.price.sell - card.price.buy) / card.price.buy * 100).toFixed()} %
                            </span>
                          </div>
                        ...
                        <div
                          ...
                          onClick={() => removeFromCart(card.assetid)}
                        >
