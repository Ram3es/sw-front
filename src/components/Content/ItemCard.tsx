...
const ItemCard = ({isTradable, timeToTrade, image, isSelected, isNoFee, price, name, type, condition, onClick, onCartAdd}: CardItem) => {
  const conditionObj = findNearestMaxValue(conditions, condition)

  return (
    <div
      className={classNames('border border-b-[3px] border-darkGrey relative min-h-[200px] overflow-hidden',
      isTradable ? "group" : '')}
      style={{
        clipPath: 'polygon(calc(100% - 8px) 0, 100% calc(0% + 8px), 100% 100%, calc(0% + 8px) 100%, 0 calc(100% - 8px), 0 0)',
      }}
    >
      <span className="absolute border-darkGrey -right-[2px] top-[3px] w-[10px] border-t rotate-45" />
      <span className="absolute border-darkGrey -left-[2px] bottom-[1px] w-[10px] border-t rotate-45" />
      <span
      ...
      <div className="p-4 pb-6 relative z-10 flex flex-col justify-between items-center w-full h-full">
        ...
        <div className="flex flex-col w-full relative mb-2">
          ...
          {isNoFee ? <span className="px-1 absolute bottom-[110%] left-0 bg-[#FFD7BC] text-[#0D0D0D] text-xs font-['Barlow']">
            0% Fee
          </span> : ""}
        </div>
        <div className="flex flex-col w-full relative mb-2">
